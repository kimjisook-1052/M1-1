...
- **4~5월, 10월**이 캠핑 적합일 비율 70%대로 가장 좋은 시기 (5월이 78%로 최고)
- **1월·7~8월·12월**은 5% 미만으로 캠핑에 부적합 (혹한 또는 폭염+장마)
...
- **캠핑 적합일 지표**를 직접 설계: 평균기온 10~25℃ & 강수량 1mm 미만인 날을 "캠핑 적합일"로 정의하고 월별 비율 계산
...
작업하면서 실제로 부딪혔던 문제와 해결 과정을 남겨둡니다 (비슷한 작업을 하실 분들께 참고가 되길 바라며).
...
> ⚠️ Windows에서 그래프 한글이 깨지면, 코드 상단의 폰트 설정 부분을 `plt.rcParams['font.family'] = 'Malgun Gothic'`으로 바꿔주세요.
...
- 기간/조건을 바꿔가며 탐색할 수 있는 웹 대시보드로 확장
- 시계열 분해(추세/계절성 분리)나 간단한 예측 모델 추가
- 세종 지점이 아닌, 합강캠핑장에 더 가까운 관측소 데이터와 비교 검증
